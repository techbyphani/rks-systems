...
export default function TasksPage() {
  ...
  const loadTasks = async () => {
    ...
    setLoading(true);
    try {
      const result = await taskService.getAll(tenant.id, filters);
      setData(result);
    } catch (error) {
  ...
  const loadStats = async () => {
    if (!tenant?.id) return;
    try {
      const statsData = await taskService.getStats(tenant.id);
      setStats({
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      await taskService.create(tenant.id, taskData);
