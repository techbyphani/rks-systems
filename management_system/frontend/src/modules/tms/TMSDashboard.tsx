  ...
  const loadData = async () => {
    ...
    try {
      const [statsData, urgentData] = await Promise.all([
        taskService.getStats(tenant.id),
        taskService.getAll(tenant.id, { priority: 'urgent', status: ['pending', 'assigned', 'in_progress'], pageSize: 5 }),
      ]);
      setStats(statsData);
      ...
      const allTasks = await taskService.getAll(tenant.id, { pageSize: 1000 });
