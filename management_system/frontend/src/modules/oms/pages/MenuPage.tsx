...
export default function MenuPage() {
  ...
  const loadItems = async () => {
    ...
    setLoading(true);
    try {
      const data = await menuService.getAll(tenant.id);
      setItems(data);
    } catch (error) {
  ...
  const handleToggleAvailability = async (item: MenuItem) => {
    ...
    try {
      await menuService.toggleAvailability(tenant.id, item.id);
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      if (editingItem) {
        await menuService.update(tenant.id, editingItem.id, values);
        message.success('Menu item updated');
      } else {
        await menuService.create(tenant.id, values);
