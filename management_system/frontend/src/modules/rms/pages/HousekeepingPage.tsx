...
export default function HousekeepingPage() {
  ...
  const loadTasks = async () => {
    ...
    setLoading(true);
    try {
      const response = await housekeepingService.getAll(tenant.id, {
  ...
  }, [selectedDate, statusFilter, tenant?.id]);
  ...
  const handleStatusUpdate = async (taskId: string, newStatus: HousekeepingTaskStatus) => {
    ...
    try {
      if (newStatus === 'in_progress') {
        await housekeepingService.start(tenant.id, taskId);
      } else if (newStatus === 'completed') {
        await housekeepingService.complete(tenant.id, taskId);
      } else if (newStatus === 'verified') {
        await housekeepingService.verify(tenant.id, taskId, 'current-user');
      } else {
        await housekeepingService.update(tenant.id, taskId, { status: newStatus });
      }
