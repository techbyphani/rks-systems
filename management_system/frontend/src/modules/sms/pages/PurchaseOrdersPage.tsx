...
export default function PurchaseOrdersPage() {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      const result = await purchaseOrderService.getAll(tenant.id, filters);
      setData(result);
    } catch (error) {
  ...
  const loadStats = async () => {
    if (!tenant?.id) return;
    try {
      const statsData = await purchaseOrderService.getStats(tenant.id);
      setStats({
  ...
  const handleStatusUpdate = async (id: string, status: PurchaseOrderStatus) => {
    ...
    try {
      await purchaseOrderService.updateStatus(tenant.id, id, status);
