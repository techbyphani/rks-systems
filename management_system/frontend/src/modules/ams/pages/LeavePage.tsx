...
export default function LeavePage() {
  ...
  const loadRequests = async () => {
    ...
    setLoading(true);
    try {
      const data = await leaveService.getAll(tenant.id, statusFilter ? { status: statusFilter } : {});
      setRequests(data);
    } catch (error) {
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      await leaveService.approve(tenant.id, id);
  ...
  const handleReject = async (id: string) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await leaveService.reject(tenant.id, id, 'Rejected by manager');
        ...
        <Form 
          ...
          onFinish={async (values) => {
            ...
            try {
              ...
              await leaveService.create(tenant.id, {
