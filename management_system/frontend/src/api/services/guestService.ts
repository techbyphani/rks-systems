...
export const guestService = {
   ...
   */
  async getAll(tenantId: string, filters: GuestFilters = {}): Promise<PaginatedResponse<Guest>> {
    await delay(300);
    requireTenantId(tenantId);
    
    let result = filterByTenant(guests, tenantId) as Guest[];
   ...
   * Create a new guest
   */
  async create(tenantId: string, data: CreateGuestDto): Promise<Guest> {
    await delay(400);
    requireTenantId(tenantId);
   ...
   */
  async update(tenantId: string, id: string, data: UpdateGuestDto): Promise<Guest> {
    await delay(400);
    requireTenantId(tenantId);
    
    const guest = findByIdAndTenant(guests, id, tenantId);
    if (!guest) {
      throw new Error('Guest not found');
    }
    
    const index = guests.findIndex(g => g.id === id && g.tenantId === tenantId);
    guests[index] = {
      ...guests[index],
      ...data,
      updatedAt: now(),
   ...
   * Get VIP guests
   */
  async getVipGuests(tenantId: string): Promise<Guest[]> {
    await delay(200);
    requireTenantId(tenantId);
    const tenantGuests = filterByTenant(guests, tenantId) as Guest[];
    return tenantGuests.filter(g => g.vipStatus !== 'none');
  },
  ...
  async getStats(tenantId: string): Promise<{
    ...
    return {
      total: tenantGuests.length,
      vipCounts: {
        none: tenantGuests.filter(g => g.vipStatus === 'none').length,
        silver: tenantGuests.filter(g => g.vipStatus === 'silver').length,
        gold: tenantGuests.filter(g => g.vipStatus === 'gold').length,
        platinum: tenantGuests.filter(g => g.vipStatus === 'platinum').length,
      },
      newThisMonth: tenantGuests.filter(g => g.createdAt.startsWith(thisMonth)).length,
    };
  },
};
