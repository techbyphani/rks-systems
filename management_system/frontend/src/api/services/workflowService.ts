...
export const workflowService = {
    ...
    const steps: WorkflowStep[] = [
      ...
      },
      {
        ...
        execute: async (reservation: Reservation, room: Room) => {
          // Create folio with reservation (hotel folio)
          const folio = await billingService.createFolio(tenantId, reservation.guestId, reservationId, roomId);
          if (!folio) {
      ...
      steps.push({
        ...
        execute: async (reservation: Reservation, room?: Room) => {
          ...
          const folio = await billingService.createFolio(
            tenantId,
            reservation.guestId,
            reservation.id, // reservationId is now optional parameter
            room?.id
          );
          if (!folio) {
            throw new WorkflowError('Failed to create folio', 'createFolio', true);
  ...
  async postCrossModuleCharge(
    ...
    // Find active folio for guest (can be hotel folio with reservation or customer folio without)
    const folios = await billingService.getAllFolios({ tenantId, guestId, status: 'open' });
    let folio: Folio | null = null;
    
    if (folios.data.length === 0) {
      // If no folio exists, create a customer folio (without reservation) for this charge
      const newFolio = await billingService.createFolio(tenantId, guestId);
      folio = await billingService.getFolioById(tenantId, newFolio.id);
    } else {
      folio = folios.data[0];
    }
